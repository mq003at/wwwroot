...
function tableSetUp(id) {
	...
	for(var k = 0; k <= year_diff; k++, year++) {
		...
		x = document.createElement("TD");
		x.setAttribute('align', 'center');
			t = document.createTextNode("Total (tauot poistettu)");
			y.appendChild(x);
			x.appendChild(t);
		
		x = document.createElement("TD");
		x.setAttribute('align', 'center');
			x.setAttribute("id", id + "hour_tt_rem");
			y.appendChild(x);
			x.innerHTML = 0;
		
		x = document.createElement("TD");
		x.setAttribute('align', 'center');
			x.setAttribute("id", id + "min_tt_rem");
			y.appendChild(x);
			x.innerHTML = 0;
	...
	else
	{
		if(dayb == daya && monthb == montha && yeara == yearb) {
			...
			if(date >= start_date && date <= end_date) {
				...
				var total_min_rem = (b.substring(8,10) * 60 + b.substring(10,12) * 1) - (a.substring(8,10) * 60 + a.substring(10,12) * 1);
				total_min_rem = Math.floor(total_min_rem) - 30;
				if (total_min_rem < 0)
					total_min_rem = 0;
				var hour_rem = Math.floor(total_min_rem/60);
				var min_rem = total_min_rem % 60;
				//hour = hour.toFixed(2);
				...
				//lunch break start
				var hour_tt_rem = hour_rem;
				var x = document.getElementById(id + "min_tt_rem");
				var min_tt_rem = min_rem * 1 + x.innerHTML * 1;
				var extra_h_rem = Math.floor(min_tt_rem/60);
				if( extra_h_rem > 0) {
					hour_tt_rem = hour_tt_rem * 1 + extra_h_rem * 1;
					min_tt_rem = min_tt_rem % 60;
				}
				x.innerHTML = min_tt_rem;
				
				x = document.getElementById(id + "hour_tt_rem");
				hour_tt_rem = hour_tt_rem * 1 + x.innerHTML * 1;
				x.innerHTML = hour_tt_rem;

...
<html>
	...
	<body>
		...
		<div id="raportit" align="center">
			<h3>RAPORTIT</h3>
			<a href="#" onclick="download_table_as_csv('time_table');">Download as CSV</a>
            </form>
			<div id="printButton" style="display:none;">
				<a id='print_btn' type="button" onclick="fnExcelReport();" class="buttona" value="Print PDF">Lataa</a>
			</div>
			...
			<table id="time_table" class="table">
				<thead>
		    	<tr>
				     <td id="current_date" data-type="number"> 
					 <?php 
					       echo date("Y-m-d") . " ";
						...
						<!--<th style="width: 30%"></th>-->
					 <td id="dateDuration" colspan="4"></td>
					</tr>
				</thead>
				<tbody id="dateTableBody" class="table" class="marginTop width80">
			...
			$(document).ready(function() {
				...
				$("#datepicker_end").on("change", function(){
					...
					if(start_date <= end_date)
						document.getElementById("dateDuration").innerHTML = start_year + "-" + start_month + "-" + start_day + "  " + end_year  + "-" + end_month + "-" + end_day;
						$('#group').html('');
						getGroups();
				});
			} );

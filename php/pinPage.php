<?php 
	session_start(); require_once('header_path.php');
?>
<!doctype html>
...
<?php
	...
	?><script>$(window.parent.document).find("#left_header").html("").append(" >> <label id='pinpage' class='header' onclick='javascript:$(\"#left_div\").attr(\"src\", \"pinPage.php\");'>TYÖAJANSEURANTA</label>");
	</script><?php
	...
	if(!isset($_SESSION['shop_id'])) {
		...
		if(isset($_SESSION['typed_pin']) && $_SESSION['typed_pin'] == $_SESSION['pin']) {
			...
			?><script>$(window.parent.document).find("#logout_la").html("<label class='header'>KIRJAUDU ULOS</label>");</script><?php
		} else {
		...
		<div id="optionArea" style="display:<?php echo $optionAreaDisplay; ?>">
			...
			<table class="noBorder center noBackground">
				...
				<tr>
					...
					<td class="tdPadding"><button type="button" class="functionButton dropbtn" id="reportin">Leimannut <br>Sisään/Ulos</button></td>
